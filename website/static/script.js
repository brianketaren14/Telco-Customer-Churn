...
cltvInput.addEventListener('input', () => { cltvVal.textContent = cltvInput.value; });
...
document.getElementById('loadSample').addEventListener('click', () => {
  ...
  cltvInput.value = 5400;
  cltvVal.textContent = 5400;
});
...
function renderSignals(payload) {
  ...
  items.push({
    label: `CLTV: ${payload.CLTV}`,
    type: payload.CLTV >= 5500 ? 'safe' : payload.CLTV <= 3000 ? 'risk' : 'warn'
  });
